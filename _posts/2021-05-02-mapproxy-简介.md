...
MapProxy 是一个 **地图服务“加速代理 + 瓦片缓存”中间层**，针对已有的 WMS、WMTS、XYZ 瓦片等地图服务，完成以下工作：
...
官方教程里已经说明：MapProxy 的配置是一个 YAML 字典，核心几块分别是：`services`、`layers`、`sources`、`caches`、`grids` 和 `globals`。
...
- 用哪个坐标系（`srs`）
- 坐标原点在哪里（`origin`）
- 分辨率、级别（可以继续配置 res / res_factor 等）
