...
一开始，你只需要输入一句 **做一个项目管理页面**，Claude Code 很快给出路由、表格、弹窗、状态管理，
甚至顺手补上了几个看起来挺完整的组件。页面能跑，交互也有，这当然比自己从零搭更快。

问题通常不会马上出现。

当你开始补充 “阶段不能直接删除”、“归档项目不能再新增任务”、“进度要按任务完成比例算，不是按阶段数算”、“以后还要接后端接口” 这些规则时，
前面那些看似合理的代码就开始变得别扭。状态散在组件里，数据关系没有讲清，删除逻辑只能靠补丁修。
AI 没有故意写错，它只是替你提前做了很多你本来没有明确做出的决定。

这正是 SDD(Spec-Driven Development) 要处理的问题。

SDD，全称 Spec-Driven Development，也就是规格驱动开发。它的核心想法很简单：
**不要让 AI 从一句模糊的 Prompt 直接跳到代码。先把需求、边界、规则和验收条件写成规格，再让 AI 根据这些规格做设计、拆任务、写代码和补测试。**

这可以简单概括为 **先写 Spec，再写代码**。

问题越早暴露，就能越早规避，在需求还没变成一堆组件、状态和接口之前。

## AI 编程的痛点

让 AI 写一个按钮、一个表单、一张列表页，通常没什么难度。
...
* 删除阶段后，阶段下的任务怎么处理；
* 项目进度到底按阶段、任务还是工时计算；
* 后续接入 API 时，现在的本地数据结构能不能平滑迁移；
* 一个 Agent 改完代码后，下一个 Agent 能不能看懂为什么这样设计。

这些问题不是语法问题，也不是组件库问题。它们属于产品规则、领域模型和工程边界。
...
一开始速度很快。功能一多，项目开始变成 **知道哪里不对，但不确定该从哪里改** 的状态。

传统开发中，代码通常被当作唯一事实来源，PRD、设计文档、测试计划往往在开发后逐渐失效。SDD 试图反过来建立关系：
...
它是在代码生成之前，把那些容易被 AI 猜错、也容易被人忽略的判断先固定下来。
在这样的情况下，**代码不是规格的附属说明，代码是规格在某种技术栈中的表达。**
...
SDD 可以理解为一种面向 AI 编程的工程协作方式。

人负责说明 **系统应该做什么、哪些规则不能突破、怎么判断完成**；AI 根据这些内容做设计和实现。
需求、计划、任务、代码和测试之间尽量保持可追溯关系。

一个成熟的 SDD 流程不要求规格永远不变。需求当然会变，设计也会变，代码更会变。
它要求的是：**变化发生时，团队知道应该先改哪一层。**

例如，产品提出一个新规则：

> 在项目管理系统中，已归档的项目不能新增阶段和任务。

这不是简单改一个 `disabled` 属性。它至少会影响：
...
如果项目没有规格，AI 往往只会在当前页面隐藏按钮。页面看起来对了，但规则并没有真正进入系统。

如果项目有 Spec，这条规则会先写进业务约束，再反映到计划、任务和测试中。

### SDD 主要解决的问题

| 传统 AI 编程问题              | SDD 的处理方式                            |
| ----------------------- | ------------------------------------ |
| AI 根据一句模糊 Prompt 自行脑补需求 | 先形成可审查的需求规格与边界条件                     |
| AI 直接进入代码，架构决策不透明       | 在 Plan 阶段显式记录技术选择和理由                 |
| 会话变长后，Agent 忘记前文约束      | 将关键上下文固化到仓库中的 Spec、Constitution、Plan |
| 代码生成很多，但无法判断是否符合真正意图    | 用验收标准、测试场景、检查表验证                     |
| 需求变化后，只能人工修改多个文件和代码     | 先更新 Spec，再重新生成 Plan、Tasks 和实现        |
| 多人协作时，产品、架构、开发理解不一致     | 用版本化 Spec 作为共享沟通载体                   |
| AI 容易过度设计               | 在 Plan Review 中检查是否引入了未要求的组件、服务或复杂性  |

总的来说，SDD 要解决的是未经约束的 Agent 容易自行假设需求、使用过时或幻觉化的 API、丢失上下文，
最终导致 **意图到代码之间形成鸿沟** 的问题。

## 什么是 Spec，为什么需要 Spec

在 SDD 语境中，Spec 不是传统意义上只写接口字段或数据库表的 **技术规格书**。
它是一个或一组**结构化、面向行为、以自然语言表达的软件功能描述**，用于指导 AI Coding Agent。
...
| 模块                   | 内容                     | 作用          |
| -------------------- | ---------------------- | ----------- |
| 背景与目标                | 要解决的问题、预期价值            | 防止实现偏离目标    |
| Scope / Out of Scope | 做什么、不做什么               | 控制范围蔓延      |
| 用户故事                 | 用户如何使用系统               | 从用户视角表达功能   |
| 功能需求                 | 系统必须具备的行为              | 形成实现边界      |
| 验收标准                 | Given / When / Then 场景 | 让需求可验证      |
| 边界条件                 | 空数据、异常输入、重复操作等         | 降低 AI 漏实现概率 |
| 非功能需求                | 性能、安全、可访问性、兼容性等        | 防止只实现“能跑”   |
| 假设与待澄清项              | 尚未确定的规则                | 防止 AI 擅自做决策 |
| 追溯关系                 | 需求编号、任务编号、测试编号         | 支持审计与变更分析   |
...
```text
FR-004：系统应允许用户编辑已有条目的标题和描述。

验收标准：
- GIVEN 用户打开已有条目的详情界面
  WHEN 用户修改标题并提交
  THEN 系统应保存新标题并在列表中立即显示更新后的内容。

- GIVEN 用户提交空标题
  WHEN 系统执行校验
  THEN 系统应阻止保存，并在标题字段附近显示错误提示。
```

这种表达方式的价值在于，它更清晰、可测试、可追溯，并且迫使需求作者补足触发条件和边界行为。
